...
class VariationalAutoencoder:
    ...
    def save_parameters(self, file_prefix):
        ...
        cPickle.dump({name: p.get_value() for name, p in self.params.items()},\
            open("CombGym/Baselines/DeepSequence/examples/params/"+"SaCas9-KKH_b0.7" + "_params.pkl", "wb")) 
        cPickle.dump({name: m.get_value() for name, m in self.m.items()}, \
            open("CombGym/Baselines/DeepSequence/examples/params/"+"SaCas9-KKH_b0.7" +"_m.pkl", "wb")) 
        cPickle.dump({name: v.get_value() for name, v in self.v.items()}, \
            open("CombGym/Baselines/DeepSequence/examples/params/"+"SaCas9-KKH_b0.7" +"_v.pkl", "wb")) 

    def load_parameters(self, file_prefix=""):
        """Load the variables in a shared variable safe way"""
        p_list = cPickle.load(open("CombGym/Baselines/DeepSequence/examples/params/"+file_prefix \
            + "_params.pkl", "rb"))
        m_list = cPickle.load(open("CombGym/Baselines/DeepSequence/examples/params/"+file_prefix \
            + "_m.pkl", "rb"))
        v_list = cPickle.load(open("CombGym/Baselines/DeepSequence/examples/params/"+file_prefix \
            + "_v.pkl", "rb"))
